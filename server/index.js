...
app.use("*", async (c, next) => {
  ...
  // Static assets and SPA routes are public (when web serving is enabled).
  // API and WebSocket routes require auth.
  const p = c.req.path;
...
if (serveWeb && webDistPath && fs.existsSync(webDistPath)) {
  ...
  // Catch-all for static files + SPA fallback (only matches non-/api/* paths)
  app.get("*", async (c) => {
    ...
    // Ensure resolved path is within webDistPath
    if (!filePath.startsWith(webDistPath)) {
      return c.text("forbidden", 403);
    }

    let ext = path.extname(filePath).toLowerCase();
    // Vite hashed assets don't end with clean extensions — try .js for unknown
